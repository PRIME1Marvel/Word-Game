...
function scan(){let f=[];for(let r=0;r<rows;r++)for(let c=0;c<cols;c++)for(const[dr,dc]of DIRS){let s="",cells=[],rr=r,cc=c;while(rr>=0&&rr<rows&&cc>=0&&cc<cols&&grid[rr][cc]){s+=grid[rr][cc];cells.push([rr,cc]);if(s.length>=minWord&&dictionary.has(s))f.push({word:s,cells:[...cells]});rr+=dr;cc+=dc}}return f}
function draw(){
  let b=$("board");
  b.innerHTML="";
  ...
  for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){
    ...
    cell.onfocus=()=>selectCell(r,c);
    cell.onclick=()=>selectCell(r,c);
    cell.oninput=e=>placeFromCell(r,c,e);
    cell.onkeydown=e=>{
      if(e.key==="Escape"){selectedCell=null;ui();return}
      if(e.key.length===1&&!/^[a-zA-Z]$/.test(e.key))e.preventDefault()
    };

    if(isSelected&&!cell.disabled)focusTarget=cell;
    b.appendChild(cell);
  }
...
function selectCell(r,c){if(!gameStarted||grid[r][c]||full())return;selectedCell={r,c};statusElText(`Cell (${r+1}, ${c+1}) locked for Player ${turn}. Type one letter.`);draw()}
function statusElText(text){$("status").textContent=text}
function placeFromCell(r,c,e){if(!gameStarted||grid[r][c]||full())return;let ch=e.target.value.toUpperCase().replace(/[^A-Z]/g,"").slice(0,1);e.target.value=ch;if(!ch)return;place(r,c,ch)}
function ui(){$("scoreA").textContent=scores.A;$("scoreB").textContent=scores.B;$("turn").textContent=turn;$("timerText").textContent=gameStarted?`${timeLeft}s left`:"30s ready";$("timerBar").style.width=(timeLeft/LIMIT*100)+"%";$("boardSize").textContent=`${rows} × ${cols} board`;$("timerBox").classList.toggle("danger",gameStarted&&timeLeft<=5);if(selectedCell&&gameStarted){$("status").textContent=`Cell (${selectedCell.r+1}, ${selectedCell.c+1}) locked for Player ${turn}. Type one letter.`}else{$("status").textContent=gameStarted?`Player ${turn}: click any empty cell and type a letter`:"Press Start Game to begin"};$("completedWords").innerHTML=lastWords.length?lastWords.map(w=>`<span class="chip">+1 ${w.word}</span>`).join(""):"No new word on the latest move.";$("history").innerHTML=history.length?history.map(h=>`<div class="move"><strong>${h.player}</strong>: ${h.text}<br><span>${h.detail}</span></div>`).join(""):"Start the game, then click any empty cell and type a letter.";let win=full()?(scores.A===scores.B?"Draw":scores.A>scores.B?"Player A":"Player B"):null;if(win){$("winnerBox").classList.remove("hidden");$("winnerBox").textContent=win==="Draw"?`It is a draw! Final score: A ${scores.A} — B ${scores.B}`:`${win} wins! Final score: A ${scores.A} — B ${scores.B}`;stop()}else $("winnerBox").classList.add("hidden");$("startBtn").disabled=gameStarted||full();document.body.classList.toggle("player-a-bg",turn==="A");document.body.classList.toggle("player-b-bg",turn==="B");draw()}
...
function place(r,c,ch){selectedCell=null;grid[r][c]=ch;let nw=scan().filter(x=>!claimedWords.has(x.word));lastWords=nw;if(nw.length){scores[turn]+=nw.length;nw.forEach(w=>claimedWords.add(w.word))}history.unshift({player:`Player ${turn}`,text:`placed "${ch}" at (${r+1}, ${c+1})`,detail:nw.length?`Scored ${nw.length}: ${nw.map(w=>w.word).join(", ")}`:"No score"});history=history.slice(0,10);turn=turn==="A"?"B":"A";timeLeft=LIMIT;ui()}
...
function importFile(file){if(!file)return;let r=new FileReader();r.onload=e=>{let words=norm(String(e.target.result||""));words.forEach(w=>dictionary.add(w));$("dictionaryInfo").textContent=`Dictionary: loaded ${words.length.toLocaleString()} words from ${file.name}. Total ${dictionary.size.toLocaleString()} words.`;history.unshift({player:"System",text:"imported dictionary file",detail:`Loaded ${words.length.toLocaleString()} words from ${file.name}.`});history=history.slice(0,10);ui()};r.readAsText(file)}
